...
import {
  serialize,
  serializeGetTransactions,
  serializeWallets,
} from "src/utils/serialize/wallet";
...
export class WalletService {
  ...
  async create(createWalletDto: CreateWalletDto) {
    ...
    if (alreadyExists)
      throw new HttpException(
        "A wallet is already registrated with this cpf",
  ...
  async findAll(queries) {
    ...
    return serializeWallets({ wallets: result });
  }
  ...
  async update(id: string, updateWalletDto: UpdateWalletDto[]) {
    ...
    const apiReturn = await this.coinService.requestCoins(updateWalletDto);
    for (let i = 0; i < apiReturn.length; i++) {
      const operate = {
        updateWalletDto: updateWalletDto[i],
        apiReturn: apiReturn[i],
      };
      await this.coinService.insertionHandler(wallet, operate);
    }
  ...
  async getTransactions(id, getTransactionDto) {
    ...
    for (let i = 0; i < coins.length; i++) {
      if (
        Object.keys(getTransactionDto).length !== 0 &&
        coins[i].coin !== getTransactionDto.coin
      )
        continue;
  ...
  async validateWallets(payload) {
    ...
    for (const id in payload) {
      ...
      if (!result) {
        throw new HttpException(
          `There's no wallet with id: ${payload[id]}`,
          HttpStatus.NOT_FOUND
        );
      }
      if (alreadyHave.indexOf(payload[id]) !== -1) {
        throw new HttpException(
          `Unable to transfer funds to your own wallet`,
          HttpStatus.BAD_REQUEST
        );
      }
